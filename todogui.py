...
class Todogui:
	def __init__(self):
		...
		def task_editor(task, oldtask):
			...
			self.label2 = gtk.Label("Add new projects with + and new contexts with @")
			...
			self.priority_combobox.connect("changed", self.priority_combo_box1_changed)
			self.project_combobox.connect("changed", self.project_combo_box1_changed)
			self.context_combobox.connect("changed", self.context_combo_box1_changed)
		...
		self.priority_combobox.connect("changed", self.priority_combo_box_changed)
		self.project_combobox.connect("changed", self.project_combo_box_changed)
		self.context_combobox.connect("changed", self.context_combo_box_changed)
	...
	def task_complete(self, task):
		...
		md = gtk.MessageDialog(None, gtk.DIALOG_MODAL|gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_QUESTION, gtk.BUTTONS_YES_NO, message)
		response = md.run()
		md.destroy()
	...
	def priority_add(self, priority_to_add):
		...
		self.task_entry.set_text(task)				
	def priority_combo_box1_changed(self, widget):
	...
	def add_new_project(self, project_to_add):
		...
		self.task_entry.set_text(task)		
	def project_combo_box1_changed(self, widget):
		model = widget.get_model()
		iter = widget.get_active_iter()
	...
	def add_new_context(self, context_to_add):
		...
		self.task_entry.set_text(task)			
	def context_combo_box1_changed(self, widget):
		model = widget.get_model()
		iter = widget.get_active_iter()
	...
	def refreshes_list(self, activate):
		self.refresh_list()
	def priority_combo_box_changed(self, widget):
		model = widget.get_model()
		iter = widget.get_active_iter()
	...
	def refresh_project_list(self):	
		...
		self.project_combobox.set_active(0)
	def project_combo_box_changed(self, widget):
		model = widget.get_model()
		iter = widget.get_active_iter()
	...
	def refresh_context_list(self):	
		...
		self.context_combobox.set_active(0)
	def context_combo_box_changed(self, widget):
		model = widget.get_model()
		iter = widget.get_active_iter()
